...
extern unsigned int aic31xx_read(struct snd_soc_codec *codec, unsigned int reg);
...
*****************************************************************************
*/
int aic31xx_eq_select = AIC31xx_SPK_EQ_SELECT;

/* aic31xx_biquad_array
 * There are two Pages related to the biquad Array. Buffer A and B and hence
 * we will maintain an array of the aic31xx_parse_data structure
 */
aic31xx_parse_data  aic31xx_biquad_array[ADAPTIVE_MAX_CONTROLS];
...
#define SOC_ADAPTIVE_CTL_AIC31xx(xname, page, reg, read_mask, write_mask) \
...
static struct snd_kcontrol_new snd_adaptive_controls[] = {
	SOC_ADAPTIVE_CTL_AIC31xx (FILT_CTL_NAME_DAC, BUFFER_PAGE_DAC, 0x1, 0x4,
				  0x4),
	SOC_ADAPTIVE_CTL_AIC31xx (COEFF_CTL_NAME_DAC, BUFFER_PAGE_DAC, 0x1, 0x2,
				  0x1),
};
...
/***************************************************************************
 * Function: aic31xx_add_biquad_controls
 *
 ...
 */
void
aic31xx_add_biquads_controls (struct snd_soc_codec *codec)
{
...
/*************************AIC31xx EQ Selection *******************************/

/***************************************************************************
 * Function: aic31xx_get_EQ_Select
 *
 * Purpose:  Add amixer kcontrols for AIC31xx EQ selection controls,
 *
 *       int - 0 on success and -1 on failure
 */
static int
aic31xx_get_EQ_Select (struct snd_kcontrol *kcontrol,
		       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = aic31xx_eq_select;
	return 0;
}

/*
 *----------------------------------------------------------------------------
 * Function : aic31xx_put_EQ_Select
 *
 * Purpose  : put routine for amixer kcontrols, write user values to registers
 *            values. Used for for AIC31xx 'MUX control' amixer controls.
 *----------------------------------------------------------------------------
 */
static int
aic31xx_put_EQ_Select (struct snd_kcontrol *kcontrol,
		       struct snd_ctl_elem_value *ucontrol)
{
	...
	if( index == -1) {
		for (index = 0; index < ARRAY_SIZE (MUX_aic31xx_controls); index++) {
			if (strstr ("Stereo_Mux_1", MUX_aic31xx_control_names[index]))
				break;
		}
		if( index == ARRAY_SIZE (MUX_aic31xx_controls)) {
			index = -1;
			return (ret_val);
		}
	}

	page = MUX_aic31xx_controls[index].control_page;

	user_value ++;

	if (user_value < AIC31xx_SPK_EQ_SELECT || user_value > AIC31xx_HP_EQ_SELECT)
		return (ret_val);

	if (index < ARRAY_SIZE (MUX_aic31xx_controls))
	{
		dprintk ("Index %d Changing to Page %d\r\n", index,
			 MUX_aic31xx_controls[index].control_page);

		aic31xx_change_page (codec, MUX_aic31xx_controls[index].control_page);

		data[1] = (u8) ((user_value >> 8) & AIC31xx_8BITS_MASK);
		data[2] = (u8) ((user_value) & AIC31xx_8BITS_MASK);

		/* start register address */
		data[0] = MUX_aic31xx_controls[index].control_base;
		...
		if (ret_val != MUX_CTRL_REG_SIZE + 1) {
			...
			aic31xx_eq_select = ucontrol->value.integer.value[0];
			ret_val = 0;
...
static const char *aic31xx_EQ_Select_Str[] = {"Speaker EQ", "Headphone EQ"};

static const struct soc_enum miniDSP_EQ_Select_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(aic31xx_EQ_Select_Str), aic31xx_EQ_Select_Str),
};

/***************************************************************************
 * Function: aic31xx_EQ_Select_mixer_controls
 *
 ...
 */
static int
aic31xx_EQ_Select_mixer_controls (struct snd_soc_codec *codec)
{
	int err;

	snd_mode_controls.name = "AIC31xx EQ Selection";
	...
	snd_mode_controls.get = aic31xx_get_EQ_Select;
	snd_mode_controls.put = aic31xx_put_EQ_Select;
...
/***************************************************************************
 * Function: aic31xx_add_EQ_mixer_controls
 *
 * Purpose: Configures the AMIXER Controls for the miniDSP run-time configuration.
 * 	    This routine configures the function pointers for the get put and the info
 *          members of the snd_mode_controls and calls the snd_ctl_add routine
 ...
 */
int
aic31xx_add_EQ_mixer_controls (struct snd_soc_codec *codec)
{
	int err;

	err = aic31xx_EQ_Select_mixer_controls (codec);
...
/***************************************************************************
 * Function: aic31xx_parse_biquad_array
 *
 * Purpose: Configures the AMIXER Controls for the miniDSP run-time configuration.
 * 	    This routine configures the function pointers for the get put and the info
 *          members of the snd_mode_controls and calls the snd_ctl_add routine
 ...
 */
int
aic31xx_parse_biquad_array (struct snd_soc_codec *codec)
{
	u16 index = 0;
	u16 count = 0;
	u16 jump;
	...
	while (index < ADAPTIVE_MAX_CONTROLS) {
		...
		if (miniDSP_D_reg_values[count].reg_off == 0) {
			aic31xx_biquad_array[index].page_num = miniDSP_D_reg_values[count].reg_val;
		...
		aic31xx_biquad_array[index].burst_array[array_index++] = regoffset = miniDSP_D_reg_values[count+1].reg_off;
		aic31xx_biquad_array[index].burst_array[array_index++] = miniDSP_D_reg_values[count+1].reg_val;
		...
		for (jump = 0; (((regoffset + array_index) < 128) &&
			...
			if (miniDSP_D_reg_values[count].reg_off != (miniDSP_D_reg_values[count - 1].reg_off + 1))
				...
				aic31xx_biquad_array[index].burst_array[array_index++] = miniDSP_D_reg_values[count].reg_val;
			}
		}

		/* Now Configure the Burst_Tranfer Length into the Array */
		aic31xx_biquad_array[index].burst_size = array_index;
		printk("Page %d has %d Coefficient Values \r\n", aic31xx_biquad_array[index].page_num,
		       aic31xx_biquad_array[index].burst_size);
	...
	while (index < ADAPTIVE_MAX_CONTROLS) {
		...
		aic31xx_change_page (codec, aic31xx_biquad_array[index].page_num);

		ret_val = i2c_master_send (i2c, &aic31xx_biquad_array[index].burst_array[0],
					   aic31xx_biquad_array[index].burst_size);

		if (ret_val != aic31xx_biquad_array[index].burst_size) {
			...
			dprintk("Wrote %d Values into Page %d Start RegOffset %d\r\n",
				aic31xx_biquad_array[index].burst_size,
				aic31xx_biquad_array[index].page_num,
				aic31xx_biquad_array[index].burst_array[0]);
		}
		index++;
	}

	aic31xx_change_page (codec, BUFFER_PAGE_DAC);
...
/*
 * Function: aic31xx_update_biquad_array
 *
 ...
 */
int
aic31xx_update_biquad_array (struct snd_soc_codec *codec, int speaker_active,
			     int playback_active)
{
	char data[2];
	struct i2c_client *i2c = codec->control_data;
	...
	if (speaker_active)
		aic31xx_eq_select = AIC31xx_SPK_EQ_SELECT;
	else
		aic31xx_eq_select = AIC31xx_HP_EQ_SELECT;
		...
		if (i2c_master_recv (i2c, data, 1) != 1) {
			 ...
			 */
				switch (dac_ram_value & 0x3) {
					case 0:
						if (aic31xx_eq_select == AIC31xx_SPK_EQ_SELECT) {
							data[1] = data[0];
						} else {
